  ...
  return (
    <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5 sm:gap-4">
      ...
      <Card className="flex min-w-0 flex-col gap-1 px-4 py-3">
        ...
        <span className="truncate text-2xl font-bold text-primary">{fmtPercent(stats.cacheHitRate)}</span>
        <span className="text-[10px] text-text-muted">{fmt(stats.totalCacheReadTokens)} cached tokens</span>
      </Card>
      <Card className="flex min-w-0 flex-col gap-1 px-4 py-3">
